...
tourney_detail = spark.table("mncaatourney_detailed")
seeds = spark.table("mncaatourney_seeds")
teams = spark.table("mteams")
conferences = spark.table("mteam_conferences")
...
# From compact results, build per-team-season aggregates
# Each row has WTeamID (winner) and LTeamID (loser)
...
# Use detailed results for advanced stats when available
detailed_wins = (
    detailed
    .groupBy("Season", "WTeamID")
    .agg(
...
# Combine into meaningful features
...
HOME_ADV = 100
INIT_ELO = 1500
SEASON_REGRESS = 0.75  # regress 25% toward mean each new season

elo = {}  # (season, team) -> elo at end of season
current_elo = {}  # team -> current elo
...
for _, row in compact_pd.iterrows():
    ...
    # New season: regress toward mean
    if season != prev_season:
        if prev_season is not None:
            # Save end-of-season Elo for every team that played
    ...
    # Home advantage
    if loc == "H":
        elo_w_adj = elo_w + HOME_ADV
    elif loc == "A":
        elo_l_adj = elo_l + HOME_ADV
        elo_w_adj = elo_w
    else:
        elo_w_adj = elo_w
        elo_l_adj = elo_l
...
elo_records = [{"Season": s, "TeamID": t, "elo_rating": e} for (s, t), e in elo.items()]
elo_df = spark.createDataFrame(pd.DataFrame(elo_records))
...
# Calculate win % over last 10 regular season games
w = Window.partitionBy("Season", "TeamID").orderBy("DayNum").rowsBetween(-9, 0)

# Combine all games from team perspective
games_as_team = (
    compact
    .select(
        F.col("Season"), F.col("DayNum"),
        F.col("WTeamID").alias("TeamID"),
        F.lit(1).alias("won"),
    )
    .union(
        compact.select(
            F.col("Season"), F.col("DayNum"),
            F.col("LTeamID").alias("TeamID"),
            F.lit(0).alias("won"),
        )
    )
)
...
last_10 = (
    games_as_team
    .withColumn("recent_wins", F.sum("won").over(w))
    .withColumn("recent_games", F.count("won").over(w))
...
# Seed format: W01, X02, Y03a, Z04b etc.
# Region letter + seed number (sometimes with a/b play-in suffix)
seed_parsed = (
    seeds
    .withColumn("seed_region", F.substring("Seed", 1, 1))
    .withColumn(
        "seed_num",
        F.regexp_extract("Seed", r"[WXYZ](\d+)", 1).cast(IntegerType()),
    )
...
# DBTITLE 1,Massey Ordinal Rankings (Season-End)
# Use the last available ranking per system per season per team
try:
    massey = spark.table("mmassey_ordinals")
    # Get the max day ordinal for each (Season, TeamID) across all systems
    massey_agg = (
        massey
        .groupBy("Season", "TeamID")
        .agg(
...
# Join all per-team-season features
team_features = (
    season_records
...
print(f"Team features shape: {team_features.count():,} rows x {len(team_features.columns)} cols")
team_features.printSchema()
...
# For every tournament game, create ordered (TeamA < TeamB) matchup row
# Label: 1 if lower-ID team won, 0 otherwise

tourney_matchups = (
    tourney
    .withColumn(
        "TeamA",
        F.when(F.col("WTeamID") < F.col("LTeamID"), F.col("WTeamID")).otherwise(F.col("LTeamID")),
    )
    .withColumn(
        "TeamB",
        F.when(F.col("WTeamID") < F.col("LTeamID"), F.col("LTeamID")).otherwise(F.col("WTeamID")),
    )
    .withColumn(
        "team1_won",
        F.when(F.col("WTeamID") < F.col("LTeamID"), F.lit(1)).otherwise(F.lit(0)),
    )
...
# DBTITLE 1,Join Team Features to Matchup Pairs
# Get features for TeamA
tf = team_features
...
fa = tf.select(
    "Season", "TeamID",
    *[F.col(c).alias(f"a_{c}") for c in feature_cols]
).withColumnRenamed("TeamID", "TeamA")

fb = tf.select(
    "Season", "TeamID",
    *[F.col(c).alias(f"b_{c}") for c in feature_cols]
).withColumnRenamed("TeamID", "TeamB")

training_data = (
    tourney_matchups
    .join(fa, ["Season", "TeamA"], "left")
    .join(fb, ["Season", "TeamB"], "left")
)

# Create difference features (TeamA - TeamB)
...
print(f"Training data: {training_data.count():,} rows x {len(training_data.columns)} cols")

# COMMAND ----------

# DBTITLE 1,Filter to Seasons With Enough Data (2003+)
training_data = training_data.filter(F.col("Season") >= 2003)
print(f"Training data (2003+): {training_data.count():,} rows")

# COMMAND ----------

# DBTITLE 1,Save Training Features Table
training_data.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(
    f"{CATALOG}.{SCHEMA}.training_features"
)
print(f"Saved to {CATALOG}.{SCHEMA}.training_features")

# Also save just the difference features + label for AutoML
diff_cols = [c for c in training_data.columns if c.startswith("diff_")]
automl_data = training_data.select("Season", "TeamA", "TeamB", *diff_cols, "team1_won")
automl_data.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable(
    f"{CATALOG}.{SCHEMA}.automl_training_data"
)
print(f"Saved AutoML table: {automl_data.count():,} rows x {len(automl_data.columns)} cols")
